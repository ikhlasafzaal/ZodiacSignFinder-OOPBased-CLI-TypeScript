...
async function getName() {
    ...
    let nameInput = await inquirer.prompt({
        ...
        validate: (input) => MyPerson.validateName(input) ? true : "Enter alphabetical letters ",
    });
...
class Zodiac extends Student {
    ...
    async getZodiacSign() {
        ...
        let birthInfo = await inquirer.prompt([
            {
                ...
                validate: (number) => this.validateDay(number) ? true : "Enter a valid day (1-31)",
            },
            {
                name: 'month',
                type: "number",
                message: chalk.rgb(64, 224, 208)("Enter your month of birth (1-12): "),
                validate: (number) => this.validateMonth(number) ? true : "Enter a valid month (1-12)",
            }
        ]);
        ...
        let compatibilitySign = await inquirer.prompt({
            ...
            validate: (input) => input.trim() !== "" ? true : "Please enter a zodiac sign",
        });
...
let signOfTheStar = ""; // Variable to store the user's zodiac sign
